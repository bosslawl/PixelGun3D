...
void UI::Render()
{
    ...
    while(!bDone) {
        ...
        if(isBlue > 0.99f && isRed == 0.0f) {
            isBlue = 1.0f;
            isGreen -= 0.01f;
        } // ugliest function ive ever seen
...
**/
void UI::GetWindow() { EnumWindows(EnumWind, NULL); }
...
**/
BOOL UI::IsWindowTargeted() { return bTargetSet; }
...
BOOL UI::IsWindowValid(const HWND hCurrentWindow)
{
    ...
    if(!IsWindowVisible(hCurrentWindow) || (IsIconic(hCurrentWindow) || IsWindowCloaked(hCurrentWindow)))
        return FALSE;
...
{
    DWORD cloaked;
    const HRESULT hr = DwmGetWindowAttribute(hCurrentWindow, DWMWA_CLOAKED, &cloaked, sizeof(cloaked));
