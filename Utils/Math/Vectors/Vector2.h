...
inline float Vector2::magnitude() const { return sqrt(x * x + y * y); }
...
inline Vector2 Vector2::clamp(const Vector2 &value, const Vector2 &min, const Vector2 &max) { return Vector2(std::clamp(value.x, min.x, max.x), std::clamp(value.y, min.y, max.y)); }
