...
public:
    ...
    inline bool operator==(const Vector3 &src) const { return (src.x == x) && (src.y == y) && (src.z == z); }

    inline bool operator!=(const Vector3 &src) const { return (src.x != x) || (src.y != y) || (src.z != z); }

    inline float Distance(const Vector3 &vOther) const { return sqrt((x - vOther.x) * (x - vOther.x) + (y - vOther.y) * (y - vOther.y) + (z - vOther.z) * (z - vOther.z)); }
    ...
    inline void NormalizeInPlace() { *this = Normalized(); }
    ...
    inline float Dot(const Vector3 &vOther) const { return (x * vOther.x + y * vOther.y + z * vOther.z); }
    ...
    inline float deltaDistance(Vector3 target_position) const { return sqrt(pow(x - target_position.x, 2) + pow(y - target_position.y, 2) + pow(z - target_position.z, 2)); }
    ...
    inline Vector3 Normalize() const
    {
        ...
        if(length != 0) {
            ...
            normalized.x = normalized.y = 0.0f;
            normalized.z                = 1.0f;
        }
...
public:
    ...
    inline Vector3Aligned(float X, float Y, float Z) { Init(X, Y, Z); }

public:
    inline explicit Vector3Aligned(const Vector3 &vOther) { Init(vOther.x, vOther.y, vOther.z); }
